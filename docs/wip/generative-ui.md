...
| Use case | Format | Why |
|---|---|---|
| Composed lists/forms/details from **our** elements, objects by reference | **A2UI** | An iframe cannot host our React components: `Row.objectRef` → tier-1 widgets (live queries, authz, panel seams), theme/typography, the shared query + live-cache loop, streamed composition — all require native rendering. "Compose our core UI elements" inside an iframe = rebuilding ui-core as template HTML |
...
fidelity, hard isolation — dominate exactly where content is foreign or
arbitrary. They are structural *disadvantages* where the ask is native
composition. Both formats stay; A2UI leads the internal investment.
