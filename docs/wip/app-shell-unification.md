...
> **Status 2026-07-13:** foundation implemented on `feat/pane-host` (unmerged): pane primitives in `packages/app-shell/src/components/pane/pane.tsx` (Pane/PaneGroup/PaneTopBar/PaneTabStrip/PaneResizeHandle + `invert` resize option), artifact store interface + in-memory placeholder impl + renderer registry + `ArtifactPane` in `packages/ai-ui/src/artifacts/`, chat route split behind a topbar toggle with persisted width; live-verified (tabs, close, resize, persistence). **Layout decision (Matthias):** the topbar spans only the main area; end panes span the full workspace height — implemented as a shell-level slot column (`workspace-end-pane.ts` + `useWorkspaceEndPaneTarget`, routes portal pane content in); the topbar button is open-only, closing lives in the pane top bar. Remaining: proper artifact backend (persisted entities — replaces store internals only), "open in pane" from tool-call cards, chat tabs (thread-binding key change), drawer `sidebar` dock migration into the pane host.
