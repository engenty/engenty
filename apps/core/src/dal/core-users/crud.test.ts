...
describe("core.users projection", () => {
  /**
   * Captures the column list handed to `.select()`. The read paths used to pass
   * `"*"`, which put every private column on the wire for anyone who could call
   * `GET /api/users` — and no type could see it, because those columns are
   * absent from `CoreUser`.
   */
  ...
  it("selects exactly the CoreUser contract fields", async () => {
    const { client, selected } = captureSelect();
    await listUsers(client as never, "t1");
    expect(selected[0].split(",").map((column) => column.trim())).toStrictEqual(
      [
        "id",
        "tenant_id",
        "email",
