...
export function registerAgentSessionRoutes(
  ...
  app.get(base, async (c) => {
    ...
    try {
      const { sessions } = await opts.aiService.sessions.listSessions({
        scope: scope.scope,
  ...
  app.post(base, async (c) => {
    ...
    const body = createSessionBodySchema.safeParse(await c.req.json());
    if (!body.success) {
      return c.json({ error: "agent_threads.invalidBody" }, 400);
    }
    try {
      const { session } = await opts.aiService.sessions.createSession({
        scope: scope.scope,
  ...
  app.get(`${base}/:threadId`, async (c) => {
    ...
    try {
      const { session } = await opts.aiService.sessions.getSession({
        scope: scope.scope,
        threadId,
      });
  ...
  app.patch(`${base}/:threadId`, async (c) => {
    ...
    const body = updateSessionBodySchema.safeParse(await c.req.json());
    if (!body.success) {
      return c.json({ error: "agent_threads.invalidBody" }, 400);
    }
    try {
      const { session } = await opts.aiService.sessions.updateSession({
        scope: scope.scope,
        threadId,
        agentId: body.data.agent_id,
        routeContext: body.data.route_context,
  ...
  app.delete(base, async (c) => {
    ...
    const hostKey = z
      ...
      .safeParse(c.req.query("host_key"));
    try {
      const { deleted } = await opts.aiService.sessions.deleteSessions({
        scope: scope.scope,
        ...(agentId.success ? { agentId: agentId.data } : {}),
        ...(hostKey.success ? { hostKey: hostKey.data } : {}),
      });
  ...
  app.get(`${base}/:threadId/messages`, async (c) => {
    ...
    try {
      const { messages } = await opts.aiService.sessions.listMessages({
        scope: scope.scope,
        threadId,
        limit: lim,
      });
  ...
  app.post(`${base}/:threadId/messages`, async (c) => {
    ...
    try {
      const { message } = await opts.aiService.sessions.appendMessage({
        scope: scope.scope,
        threadId,
        role: body.data.role,
  ...
  app.post(`${base}/:threadId/generate`, async (c) => {
    ...
    try {
      const { text, message } = await opts.aiService.sessions.generate({
  ...
  app.delete(`${base}/:threadId`, async (c) => {
    ...
    try {
      const { deleted } = await opts.aiService.sessions.deleteSession({
        scope: scope.scope,
        threadId,
      });
      if (!deleted) {
