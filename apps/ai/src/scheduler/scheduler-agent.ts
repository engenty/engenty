// The no-op agent every scheduler heartbeat is attached to.
//
// The Mastra heartbeat worker requires a REGISTERED agent — a missing agent
// makes the worker delete the schedule (self-clean). Engenty heartbeats never
...
export function createSchedulerAgent(): Agent {
  return new Agent({
    ...
    description:
      "Internal no-op anchor for Engenty scheduler heartbeats. Never runs.",
    instructions:
