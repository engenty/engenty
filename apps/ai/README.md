...
- **`AI_CHAT_MODEL`** and **`AI_ROUTING_MODEL`** — optional **seed** values for the `ai.model_binding` roles, read only while that table is empty; after first boot the bindings console is the source of truth. The Engenty Copilot supervisor uses the routing role; specialists and sub-agents use the chat roles. (The legacy `AI_COORDINATOR_MODEL` alias was removed 2026-08-04.)
