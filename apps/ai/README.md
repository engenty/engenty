...
- **`AI_CHAT_MODEL`** and **`AI_ROUTING_MODEL`** (legacy **`AI_COORDINATOR_MODEL`**) — optional process defaults when tenant settings from `/settings/ai` are not set. The Engenty Copilot supervisor uses the routing model; specialists and sub-agents use the chat model.
