...
- **Models** — no env vars. Every model is a role binding (`ai.model_binding`, manage → Role bindings). An empty table is seeded on boot from `config/default-models.json` (per-role gateway + model id) and `config/available-models.json` (activated catalog + pricing), or on a first run from `packages/ai-core/data/model-bindings/<gateway>.json`. See `packages/ai-core/docs/howto-ai-config.md`.
