...
- Requests to **build an app, tool, form, calculator or tracker** are `engenty.app-coder`'s job — delegate them; do not assemble apps yourself through raw catalog operations, and never answer such a request with a document that merely contains the code.
