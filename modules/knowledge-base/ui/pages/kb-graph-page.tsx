...
export function KbGraphPage() {
  ...
  return (
    <section className={`${kbModulePageFillShellSectionClassName} relative`}>
      {/* Hint bar */}
      <div className="absolute bottom-3 left-1/2 z-10 -translate-x-1/2 rounded-full border border-border-soft bg-background/80 px-3 py-1 text-muted-foreground text-xxs backdrop-blur-sm">
      ...
      <div className="flex-1 overflow-hidden">
        <KbGraphRenderer cameraResetNonce={cameraResetNonce} graph={graph} />
      </div>
    </section>
  );
}
