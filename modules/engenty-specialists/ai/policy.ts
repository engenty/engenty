// What a hired engenty runs with: the floor unioned onto its row, the skills
// a run may open, the lanes its tools ride with, and the server-side go-live
// gate for coordinator-created agents. Pure over the floor constants — apps/ai
// applies it at assembly (assemble-dynamic-agent, effective-capabilities,
// session-service) and in the propose/hire tools.
//
// Axis is the specialist's declared tools/skills + whether this is a NEW hire
// with a known space — not the tenant operation-approval mode (Axis B).
...
/**
 * Only a NEW hire in a known Space that asks for nothing beyond the floor
 * goes live without a person: anything wider — extra tools, a skill, a
 * revision of an existing row — suspends on an Approve card in the propose
 * tool. The gate reads the REQUESTED tools, not the floored ones.
 */
