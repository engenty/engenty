...
import type { TaskSettings, TaskStatusDefinition } from "../schema/types.js";
...
function parseDefinitionRow(row: unknown): TaskStatusDefinition | null {
...
export function normalizeTaskStatusDefinitionsFromStorage(
  ...
  const ordered: TaskStatusDefinition[] = [];
...
): Pick<TaskSettings, "default_task_statuses" | "task_status_definitions"> {
  return {
...
export function mergeTaskStatusDefinitionsFromPayload(
  payload: TaskStatusDefinition[]
): TaskStatusDefinition[] {
  const fromClient = payload
    .map((row) => parseDefinitionRow(row))
    .filter((x): x is TaskStatusDefinition => x !== null);
