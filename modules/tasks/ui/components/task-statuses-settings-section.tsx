...
interface TaskStatusesSettingsSectionProps {
  onDefinitionsChange: (definitions: TaskStatusDefinition[]) => void;
...
export function TaskStatusesSettingsSection({
  ...
  const updateRow = (index: number, patch: Partial<TaskStatusDefinition>) => {
  ...
  const insertRowBelow = (index: number) => {
    const id = `extra_${Date.now()}`;
    setNewlyAddedId(id);
    const newRow: TaskStatusDefinition = {
      id,
      label: "",
      color: "slate",
      locked: false,
  ...
  return (
    <SettingsFormSection
      description={t("settings.taskStatusesDesc")}
      title={t("settings.taskStatusesTitle")}
    >
      <DndContext
