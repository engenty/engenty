/** Structured contact person name: format, derive display_name, split legacy strings. */
...
export type ContactNameWriteInput = Partial<
  Omit<ProfileNameParts, "full_name_override">
> & {
  display_name?: string | null;
  display_name_override?: string | null;
};

export type ContactNameParts = Omit<ProfileNameParts, "full_name_override"> & {
  display_name_override: string | null;
};

function contactPartsFromProfile(parts: ProfileNameParts): ContactNameParts {
  const { full_name_override, ...structured } = parts;
  return { ...structured, display_name_override: full_name_override };
}

export function resolveContactNameForWrite(input: ContactNameWriteInput): {
  parts: ContactNameParts;
  display_name: string;
} {
  const resolved = resolveProfileNameForWrite({
    ...
    phonetic_name: input.phonetic_name,
    birth_name: input.birth_name,
    full_name_override: input.display_name_override,
    full_name: input.display_name,
  });
  return {
    parts: contactPartsFromProfile(resolved.parts),
    display_name: resolved.full_name,
  };
}

export function isContactNameWriteValid(input: ContactNameWriteInput): boolean {
  return isProfileNameWriteValid({
    name_prefix: input.name_prefix,
    first_name: input.first_name,
    middle_name: input.middle_name,
    last_name: input.last_name,
