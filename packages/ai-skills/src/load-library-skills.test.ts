...
describe("library skills pack", () => {
  ...
  it("holds no Space playbooks — those ship with engenty-specialists", () => {
    // The copilot's and hired engenties' shared playbooks (routines,
    // space-data, chief-of-staff, …) are seeded by name from the module, so
    // the library must not carry a copy: the seed refuses a name collision.
    expect(librarySkillNamesByCategory().spaces).toBeUndefined();
    expect(listLibrarySkills().map((skill) => skill.name)).not.toContain(
      "routines"
    );
  });
});
