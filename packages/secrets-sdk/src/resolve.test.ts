...
describe("canReadSecret — explicit grant (either principal kind)", () => {
  it("a direct secret grant beats every scope rule", async () => {
    ...
    expect(await can(user("nobody"), secret({ owner_scope: "user", owner_id: "someone" }), deps)).toBe(true);
...
describe("canReadSecret — user × owner_scope truth table", () => {
  ...
  it("client scope: iff assigned to the client (via project membership)", async () => {
    ...
    expect(await can(user(), s, makeDeps({ isAssignedToClient: vi.fn(async () => true) }))).toBe(true);
    expect(await can(user(), s, makeDeps({ isAssignedToClient: vi.fn(async () => false) }))).toBe(false);
  });

  it("project scope: iff a member of that project", async () => {
    const s = secret({ owner_scope: "project", owner_id: "p1" });
    expect(await can(user(), s, makeDeps({ isProjectMember: vi.fn(async () => true) }))).toBe(true);
    expect(await can(user(), s, makeDeps({ isProjectMember: vi.fn(async () => false) }))).toBe(false);
  });
  ...
  it("client-scope check queries the client, not the project, source", async () => {
    ...
    await can(user("u1"), secret({ owner_scope: "client", owner_id: "c7" }), makeDeps({ isAssignedToClient, isProjectMember }));
...
describe("canReadSecret — agent path (no scope fallthrough)", () => {
  it("allows exactly the secret named in a goal grant", async () => {
    ...
    expect(await can(agent("a1", "g1"), secret({ id: "s2" }), deps)).toBe(false);
  });
  ...
  it("denies an agent that only has scope membership (agents never inherit scope)", async () => {
    ...
    expect(await can(agent("a1", "g1"), secret({ owner_scope: "client", owner_id: "c1" }), deps)).toBe(false);
    expect(await can(agent("a1", "g1"), secret({ owner_scope: "project", owner_id: "p1" }), deps)).toBe(false);
    expect(await can(agent("a1", "g1"), secret({ owner_scope: "tenant", owner_id: TENANT }), deps)).toBe(false);
  });
  ...
  it("no goal id ⇒ no goal-grant lookup ⇒ denied without a direct grant", async () => {
    ...
    expect(await can(agent("a1", null), secret({ id: "s1" }), deps)).toBe(false);
  ...
  it("goal-grant lookup is scoped to (tenant, goal, agent)", async () => {
    ...
    await can(agent("a1", "g1"), secret({ id: "s1" }), makeDeps({ listGoalGrantCapabilities }));
