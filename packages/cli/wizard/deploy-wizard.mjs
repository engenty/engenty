 ...
 *
 *   npx engenty deploy             # from anywhere — works in ./engenty-deploy
 *   pnpm engenty deploy            # inside a checkout — works in deploy/
...
//
// Inside a checkout (the current directory is under a pnpm workspace with a
// deploy/ directory) the files are the checkout's: deploy/.env, the migration
// sources for the schema list, deploy/scripts/migrate.sh. Anywhere else —
// `npx engenty deploy` on the server — it works in ./engenty-deploy: the env
// file plus the compose files copied from the package's templates, and the
// schema list from the release manifest. The server needs no clone.
// ─────────────────────────────────────────────────────────────────────────────
...
const CHECKOUT = findCheckoutRoot(process.cwd());
const DEPLOY_DIR = CHECKOUT
  ? `${CHECKOUT}/deploy`
  : `${process.cwd()}/engenty-deploy`;
const DEPLOY_LABEL = CHECKOUT ? "deploy" : "engenty-deploy";
const ENV_PATH = `${DEPLOY_DIR}/.env`;
const ENV_LABEL = `${DEPLOY_LABEL}/.env`;
const CONFIG_TOML = CHECKOUT ? `${CHECKOUT}/supabase/config.toml` : null;
// The compose files a standalone run copies next to .env. Published packages
// carry them under templates/ (public image names); a workspace build run
// from outside its checkout falls back to the checkout's deploy/.
...
function migrateHint() {
  return CHECKOUT
    ? "supabase link --project-ref " +
        (state.supa.ref || "<ref>") +
        " && bash deploy/scripts/migrate.sh"
    : "npx engenty deploy migrate  (reads SUPABASE_DB_URL from " +
        ENV_LABEL +
        ")";
}

/** Standalone only: the compose files the server runs, next to the .env. */
function writeDeployTemplates() {
  const written = [];
  for (const name of TEMPLATE_FILES) {
    const source = `${TEMPLATE_DIR}/${name}`;
...
function exposedSchemasFromConfig() {
  if (!(CONFIG_TOML && existsSync(CONFIG_TOML))) {
    return null;
  }
...
async function resolveExposedSchemas() {
  if (!CHECKOUT) {
    const manifestPath = `${PACKAGE_ROOT}/release-manifest.json`;
    if (existsSync(manifestPath)) {
      const manifest = JSON.parse(readFileSync(manifestPath, "utf8"));
      return {
        schemas: manifest.schemas,
        source: `release ${manifest.version}`,
      };
    }
    return { schemas: null, source: null };
  }
  try {
    const [{ resolveMigrationOwners }, { composeApiSchemasFromOwners }] =
      await Promise.all([
        import(pathToFileURL(`${CHECKOUT}/scripts/lib/migration-owners.mjs`)),
        import(pathToFileURL(`${CHECKOUT}/scripts/supabase-sync-lib.mjs`)),
      ]);
    const schemas = composeApiSchemasFromOwners(
      resolveMigrationOwners(CHECKOUT)
    );
    if (schemas.length > 0) {
...
async function stepWelcome() {
  ...
  note(`${ENV_LABEL}, and brings the stack up — pausing before every change.`);
  out();
  if (DRY) {
    warnLine(
  ...
  infoLine(
    CHECKOUT
      ? `Checkout:       ${paint(CHECKOUT, c.cyan)}`
      : `Deploy dir:     ${paint(DEPLOY_DIR, c.cyan)} ${paint("(no checkout here — the compose files are written next to .env)", c.dim)}`
  );
  infoLine(
    `Schemas found:  ${
      state.schemas
        ...
        : paint(
            CHECKOUT
              ? "could not read this checkout — will prompt"
              : "no release manifest next to the CLI — will prompt",
            c.yellow
          )
    }`
  );
...
async function verifySupabaseWiring() {
  ...
  try {
    probes = await import(
      new URL("../lib/supabase-probes.mjs", import.meta.url)
    );
  } catch (err) {
...
async function stepSupabase() {
  ...
  if (state.target.supabase === "cloud") {
    note(`Manual fallback (if SUPABASE_DB_URL is unset): ${migrateHint()}`);
  }
}
...
async function stepEnv() {
  ...
  out();
  const choice = await select(`Write this to ${ENV_LABEL}?`, [
    {
  ...
  if (choice.value === "print") {
    ...
    if (DRY) {
      warnLine(`DRY-RUN: would write ${ENV_LABEL}`);
    } else {
      mkdirSync(DEPLOY_DIR, { recursive: true });
      writeFileSync(ENV_PATH, envToText(e), "utf8");
      okLine(
        `Wrote ${paint(ENV_LABEL, c.cyan)} (${Object.keys(e).filter((k) => !k.startsWith("#")).length} keys)`
      );
      if (!CHECKOUT) {
        const files = writeDeployTemplates();
        if (files.length > 0) {
          okLine(
            `Wrote ${paint(files.map((f) => `${DEPLOY_LABEL}/${f}`).join(", "), c.cyan)}`
          );
        } else {
          warnLine(
            `No compose templates found at ${TEMPLATE_DIR} — copy deploy/docker-compose.prebuilt.yaml from the repository yourself.`
          );
        }
      }
    }
...
function stepServer() {
  ...
  const compose = `docker compose -f ${DEPLOY_LABEL}/docker-compose.prebuilt.yaml --env-file ${ENV_LABEL}`;
  ...
  if (CHECKOUT) {
    ...
    note("Or skip the clone: run `npx engenty deploy` on the server itself.");
  } else {
    out(`  ${paint(`1. Put ${DEPLOY_LABEL}/ on the server`, c.bold)}`);
    note("The .env and the compose files this wizard wrote — nothing else.");
    note("Ran it on the server already? Then it is in place.");
  }
  out();
...
function stepDone() {
  ...
  note(
    `• Migrations: set SUPABASE_DB_URL for auto-migrate on deploy, or apply once with ${migrateHint()}`
  );
  note(
    "• Point DNS for " +
...
function help() {
  banner();
  out(
    "  Usage: npx engenty deploy [--dry-run] [--no-color]   (in a checkout: pnpm engenty deploy)"
  );
  out();
  out(
