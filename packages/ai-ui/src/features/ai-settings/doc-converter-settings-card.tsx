  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-4">
        <Label
          className="shrink-0 sm:w-32 md:w-40"
          htmlFor="doc-converter-mistral-model"
        >
          {t("docConverter.mistralModelLabel")}
        </Label>
        <Input
          className="h-8 min-w-0 flex-1"
          id="doc-converter-mistral-model"
          onChange={(e) =>
            updateSettings("doc_converter", {
              ...dc,
              mistral_model: e.target.value.trim() || null,
            })
          }
          placeholder={DEFAULT_MISTRAL_MODEL}
          value={dc.mistral_model ?? ""}
        />
      </div>
      <p className="text-muted-foreground text-xs">
        {t("docConverter.mistralModelHelp")}
      </p>
          ...
          <ul className="mt-1 list-inside list-disc space-y-0.5">
            ...
            <li>
              ...
              {availability?.mistral
                ? t("docConverter.backendReady")
                : t("docConverter.backendMissingMistral")}
            </li>
            <li>
              Gemini (AI Gateway):{" "}
              {availability?.gemini
