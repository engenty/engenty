...
export interface ActionRunStatusState {
...
export interface UseActionRunStatusResult {
  /** Stop the run; null when there's no live (running) run to stop. */
  cancel: (() => void) | null;
  isCancelling: boolean;
  state: ActionRunStatusState | null;
}
 ...
 */
export function useActionRunStatus(
  runId: string | null
): UseActionRunStatusResult {
  const [state, setState] = useState<ActionRunStatusState | null>(null);
  ...
  useEffect(() => {
    ...
    const commit = (patch: Partial<ActionRunStatusState>) => {
      setState((prev) => ({
