// Single status→icon/color mapping for agent sessions, shared by the copilot
// session list, the agents-workspace admin views, and headers. Labels are
...
import type { AppsAiThreadStatus } from "../../ag-ui/apps-ai/apps-ai-session-api.js";

export type SessionStatusIconSize = "sm" | "md";

export interface SessionStatusIconProps {
  className?: string;
  /** Accessible label for the status (caller-translated). */
  label?: string;
  size?: SessionStatusIconSize;
  status: AppsAiThreadStatus;
}

const iconSizeClassName: Record<SessionStatusIconSize, string> = {
  md: "size-3.5",
  sm: "size-3",
};
...
export function sessionStatusBadgeClassName(
  status: AppsAiThreadStatus
): string {
