/**
 * Hybrid search over indexed chat sessions (conversation history).
...
export const CHAT_SESSION_INDEX_HEALTH_METHOD =
  "ai_chat_sessions_index_health" as const;

export const chatSessionIndexHealthInputSchema = z.object({});

export type ChatSessionIndexHealthInput = z.infer<
  typeof chatSessionIndexHealthInputSchema
>;

/** Registered host operation id for search. */
export const CHAT_SESSION_SEARCH_METHOD = "ai_chat_sessions_search" as const;
export const CHAT_SESSION_SEARCH_TOOL_ID = "chatSessionSearch" as const;
...
export const aiChatsSearchInputSchema = z.object({
  ...
  query: z
    ...
    .min(1)
    .describe(
      "Natural-language query over indexed chat session content (messages, titles, summaries)."
    ),
  strategy: z
...
const chatSessionSearchToolDescription =
  "Search the current user's indexed orchestrator chat sessions (hybrid lexical + semantic). The tool checks index completeness once per chat session and returns a short notice when the index is missing or incomplete; still returns whatever matches exist so you can answer from partial history. Not for browser or auth sessions.";

export interface ChatSessionSearchToolDefinition {
  description: string;
  execute: (input: AiChatsSearchInput) => Promise<unknown>;
  id: typeof CHAT_SESSION_SEARCH_TOOL_ID;
...
async function loadIndexHealth(
  ...
  const raw = await call(CHAT_SESSION_INDEX_HEALTH_METHOD, {});
...
async function runChatSessionSearchTool(
  ...
  try {
    ...
    const searchRaw = await call(CHAT_SESSION_SEARCH_METHOD, parsed);
    ...
    return {
      error:
        error instanceof Error ? error.message : "Chat session search failed",
    };
  }
}
...
export function buildMastraChatSessionSearchTool<TTool>(
  createTool: (definition: ChatSessionSearchToolDefinition) => TTool,
  ctx: ToolExecutionContext
): TTool {
  return createTool({
    description: chatSessionSearchToolDescription,
