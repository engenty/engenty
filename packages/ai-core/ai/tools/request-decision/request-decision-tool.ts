...
export const requestDecisionToolDefinition: RequestDecisionToolDefinition = {
  id: "requestDecision",
  description:
    "Render an interactive chooser widget for the user. Use this instead of plain numbered or bulleted text when the user asks to choose, approve/decline, or select from up to 6 options. If the user asks for a chooser with a count but omits exact options, infer reasonable low-risk choices when safe; ask for clarification only when the choices depend on private app data, business rules, or a risky action.",
