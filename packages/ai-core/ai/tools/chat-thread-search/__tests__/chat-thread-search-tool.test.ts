...
describe("buildChatThreadSearchTool", () => {
  it("calls index health at most once per cache key for repeated searches", async () => {
    ...
    const call = vi.fn(async (method: string, input: unknown) => {
      if (method === "ai_chat_threads_index_health") {
        return healthSpy();
      }
      if (method === "ai_chat_threads_search") {
    ...
    const tool = buildChatThreadSearchTool(ctx);
  ...
  it("builds a Mastra-compatible tool with the shared execution path", async () => {
    ...
    const createTool = vi.fn((definition: ChatThreadSearchToolDefinition) => ({
      ...definition,
    }));
    const ctx: ToolExecutionContext = {
      action: "test",
    ...
    const tool = buildMastraChatThreadSearchTool(createTool, ctx);
    ...
    expect(call).toHaveBeenCalledWith("ai_chat_threads_search", {
      query: "previous plan",
    });
  });
});
