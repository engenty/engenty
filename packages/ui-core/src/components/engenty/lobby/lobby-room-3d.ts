...
export function mountLobbyRoom(
  ...
  const toon = (hex: string, map?: Texture | null) =>
    track(
      new MeshToonMaterial({
        color: map ? 0xff_ff_ff : new Color(hex),
        gradientMap: grad,
        map: map ?? null,
      })
    );
  ...
  scene.add(new AmbientLight(0xff_ff_ff, 2.2));
  const sun = new DirectionalLight(0xff_f4_e6, 2.2);
  ...
  const frame = (now: number) => {
    ...
    for (const c of clouds) {
      ...
      if (c.group.position.x > c.to) {
        c.group.position.x = c.from;
      }
    }
