...
interface MultiSelectProps
	...
	/**
	 * If true, shows the clear-all button when options are selected.
	...
	/**
	 * Custom element to render inside the PopoverTrigger instead of the default button content.
	 * Optional.
	 */
...
export const MultiSelect = React.forwardRef<MultiSelectRef, MultiSelectProps>(
	(
		...
		const getBadgeAnimationClass = () => {
			const badgeAnim = animationConfig?.badgeAnimation ?? "none";
			if (badgeAnim === "none") {
				return "";
			}
			switch (badgeAnim) {
				...
				<Popover
					...
					<PopoverTrigger asChild>
						<Button
							...
							className={cn(
								variant === "ghost"
									? "flex min-h-0 h-auto items-center justify-between gap-1 bg-transparent hover:bg-transparent [&_svg]:pointer-events-auto"
								...
								variant === "ghost" && "border-0 p-0 rounded-none",
							...
							{triggerElement ? (
								...
								<div className="flex justify-between items-center w-full">
											...
											.map((value) => {
												...
												return (
													<Badge
														key={value}
														className={cn(
															"inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 leading-none",
														...
														<div
															...
															className="ml-1 inline-flex h-5 w-5 items-center justify-center rounded-full p-0.5 text-current/80 hover:bg-white/20 hover:text-current focus:outline-none focus:ring-1 focus:ring-white/50">
															<XCircle
																className={cn(
																	"h-3.5 w-3.5",
																	responsiveSettings.compactMode &&
									...
									<div className="flex items-center justify-between">
										{showClear && (
											<>
												<div
													...
													className="flex h-4 w-4 cursor-pointer items-center justify-center rounded-sm text-muted-foreground hover:text-foreground focus:outline-none focus:ring-1 focus:ring-ring/50 focus:ring-offset-1 focus:ring-offset-background mx-2">
										...
										{variant === "ghost" ? (
											<Pencil
												className="h-4 w-4 mx-2 cursor-pointer text-muted-foreground shrink-0"
												aria-hidden="true"
											/>
										) : (
											<ChevronDown
								...
								<div className="flex items-center justify-between w-full mx-auto">
									...
									{variant === "ghost" ? (
										<Pencil className="h-4 w-4 cursor-pointer text-muted-foreground mx-2 shrink-0" />
									) : (
