...
function EmojiPickerEmoji({
  ...
  return (
    <button
      {...props}
      className={cn(
        "flex size-7 items-center justify-center rounded-sm text-base outline-none data-[active]:bg-accent",
...
export type EmojiPickerContentLabels = {
  empty?: string;
};
...
function EmojiPickerContent({
  ...
  labels?: EmojiPickerContentLabels;
}) {
  return (
...
export type EmojiPickerFooterLabels = {
  selectEmoji?: string;
};
...
function EmojiPickerFooter({
  ...
  labels?: EmojiPickerFooterLabels;
}) {
  return (
    <div
      className={cn(
        ...
        {({ emoji }) =>
          emoji ? (
            ...
            <span className="ml-1.5 flex h-7 items-center truncate text-muted-foreground text-xs">
              {labels?.selectEmoji ?? "Select an emoji…"}
            </span>
          )
        }
      </EmojiPickerPrimitive.ActiveEmoji>
