...
const selectContentVariants = cva(
  "relative isolate z-50 max-h-(--available-height) w-(--anchor-width) min-w-36 origin-(--transform-origin) overflow-x-hidden overflow-y-auto p-1 text-popover-foreground transition-[scale,opacity] duration-100 ease-out data-[align-trigger=true]:transition-none data-starting-style:scale-95 data-starting-style:opacity-0 data-ending-style:scale-95 data-ending-style:opacity-0 data-[align-trigger=true]:data-starting-style:scale-100 data-[align-trigger=true]:data-starting-style:opacity-100 data-[align-trigger=true]:data-ending-style:scale-100 data-[align-trigger=true]:data-ending-style:opacity-100",
  {
    variants: {
      variant: {
        default: "ui-canvas-floating rounded-md",
        glass:
          "ui-canvas-glass rounded-2xl [box-shadow:var(--shadow-ember-overlay)]",
      },
    },
    defaultVariants: {
      variant: "default",
...
function SelectItem({
  ...
  return (
    <SelectPrimitive.Item
      ...
      className={cn(
        "relative flex w-full cursor-default items-center gap-2 rounded-sm py-1.5 pr-8 pl-2 text-sm outline-hidden select-none focus:bg-accent focus:text-accent-foreground not-data-[variant=destructive]:focus:**:text-accent-foreground data-disabled:pointer-events-none data-disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 *:[span]:last:flex *:[span]:last:items-center *:[span]:last:gap-2 [[data-variant=glass]_&]:rounded-lg",
        className
      )}
      {...props}
    >
      <SelectPrimitive.ItemText className="flex min-w-0 flex-1 gap-2 text-left">
        {children}
