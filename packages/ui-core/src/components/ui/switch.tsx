  ...
  return (
    ...
    >
      <SwitchPrimitive.Thumb
        ...
        // Checked offset = track content width (root width minus its 1px
        // borders) minus the thumb's own width, so the thumb lands flush
        // against the far end. `calc(100% - 2px)` only worked when 1rem
        // happened to be 16px; this app's root is 14px, which left the thumb
        // short of the right edge.
        className="pointer-events-none block rounded-full bg-background ring-0 transition-transform group-data-[size=default]/switch:size-4 group-data-[size=sm]/switch:size-3 group-data-[size=default]/switch:data-checked:translate-x-[calc(30px-100%)] group-data-[size=sm]/switch:data-checked:translate-x-[calc(22px-100%)] dark:data-checked:bg-primary-foreground group-data-[size=default]/switch:data-unchecked:translate-x-0 group-data-[size=sm]/switch:data-unchecked:translate-x-0 dark:data-unchecked:bg-foreground"
      />
    </SwitchPrimitive.Root>
  )
}
